...
class XianyuLive:
    ...
    def _extract_order_id(self, message: dict) -> str:
        ...
        try:
            ...
            logger.debug(
                f"【{self.cookie_id}】🔍 message['1'] keys: {list(message_1.keys()) if message_1 else 'None'}"
            )

            # 检查message['1']['6']的结构
            message_1_6 = message_1.get("6", {}) if message_1 else {}
            logger.debug(
                f"【{self.cookie_id}】🔍 message['1']['6'] keys: {list(message_1_6.keys()) if message_1_6 else 'None'}"
            )

            # 方法1: 从button的targetUrl中提取orderId
            content_json_str = (
                message.get("1", {}).get("6", {}).get("3", {}).get("5", "")
            )
            if content_json_str:
                try:
                    ...
                    if target_url:
                        ...
                        if order_match:
                            ...
                            logger.info(
                                f"【{self.cookie_id}】✅ 从button提取到订单ID: {order_id}"
                            )
                    ...
                    if not order_id:
                        ...
                        if main_target_url:
                            ...
                            if order_match:
                                order_id = order_match.group(1)
                                logger.info(
                                    f"【{self.cookie_id}】✅ 从main targetUrl提取到订单ID: {order_id}"
                                )
            ...
            if not order_id and content_json_str:
                try:
                    content_data = json.loads(content_json_str)
                    dynamic_target_url = (
                        content_data.get("dynamicOperation", {})
                        .get("changeContent", {})
                        .get("dxCard", {})
                        .get("item", {})
                        .get("main", {})
                    ...
                    if dynamic_target_url:
                        ...
                        order_match = re.search(
                            r"order_detail\?id=(\d+)", dynamic_target_url
                        )
                        if order_match:
                            order_id = order_match.group(1)
                            logger.info(
                                f"【{self.cookie_id}】✅ 从order_detail提取到订单ID: {order_id}"
                            )
                except Exception as parse_e:
        ...
        try:
            ...
            try:
                ...
                if delivery_content:
                    ...
                    logger.info(
                        f"[{msg_time}] 【自动发货】已向 {user_url} 发送发货内容"
                    )
                    await self.send_delivery_failure_notification(
                        send_user_name, send_user_id, item_id, "发货成功"
                    )
                else:
                    logger.warning(
                ...
                # 发送自动发货异常通知
                await self.send_delivery_failure_notification(
                    send_user_name, send_user_id, item_id, f"自动发货处理异常: {str(e)}"
                )
            ...
            if hasattr(self, "cookie_id") and self.cookie_id:
                try:
                    ...
                    db_manager.save_cookie(
                        self.cookie_id, self.cookies_str, current_user_id
                    )
    ...
    async def fetch_item_detail_from_api(self, item_id: str) -> str:
        ...
        try:
            ...
            # 从配置获取API地址和超时时间
            api_base_url = auto_fetch_config.get(
                "api_url", "https://selfapi.zhinianboke.com/api/getItemDetail"
            )
                ...
                async with session.get(api_url) as response:
                    if response.status == 200:
                        ...
                        if result.get("status") == "200" and result.get("data"):
                            ...
                            logger.info(
                                f"成功获取商品详情: {item_id}, 长度: {len(item_detail)}"
                            )
                            logger.debug(f"商品详情内容: {item_detail[:200]}...")
                            return item_detail
                        else:
                            logger.warning(
                                f"API返回状态异常: {result.get('status')}, message: {result.get('message')}"
                            )
                            return ""
                    else:
    ...
    async def save_items_list_to_db(self, items_list):
        ...
        try:
            ...
            for item in items_list:
                ...
                has_detail = (
                    existing_item
                    and existing_item.get("item_detail")
                    and existing_item["item_detail"].strip()
                )
            ...
            if items_need_detail:
                ...
                if auto_fetch_config.get("enabled", True):
                    logger.info(
                        f"发现 {len(items_need_detail)} 个商品缺少详情，开始获取..."
                    )
                    detail_success_count = await self._fetch_missing_item_details(
                        items_need_detail
                    )
                    logger.info(
                        f"成功获取 {detail_success_count}/{len(items_need_detail)} 个商品的详情"
                    )
                else:
                    logger.info(
                        f"发现 {len(items_need_detail)} 个商品缺少详情，但自动获取功能已禁用"
                    )
    ...
    async def get_item_info(self, item_id, retry_count=0):
        ...
        if retry_count > 0:
            old_token = (
                self.cookies.get("_m_h5_tk", "").split("_")[0]
                if self.cookies.get("_m_h5_tk")
                else ""
            )
            logger.info(
                f"重试第{retry_count}次，强制刷新token... 当前_m_h5_tk: {old_token}"
            )
            ...
            if (
                ...
                logger.info(
                    f"Token过期或不存在，刷新token... 当前_m_h5_tk: {old_token}"
                )
                await self.refresh_token()
                new_token = (
                    self.cookies.get("_m_h5_tk", "").split("_")[0]
    ...
    async def get_default_reply(
        self, send_user_name: str, send_user_id: str, send_message: str
    ) -> str:
        """获取默认回复内容，支持变量替换"""
        try:
            from db_manager import db_manager
                ...
                for keyword, reply, keyword_item_id in keywords:
                    if (
                        ...
                        try:
                            ...
                            logger.info(
                                f"商品ID关键词匹配成功: 商品{item_id} '{keyword}' -> {formatted_reply}"
                            )
                            return formatted_reply
                        except Exception as format_error:
                            logger.error(
            ...
            for keyword, reply, keyword_item_id in keywords:
                if not keyword_item_id and keyword.lower() in send_message.lower():
                    ...
                    try:
                        ...
                        logger.info(
                            f"通用关键词匹配成功: '{keyword}' -> {formatted_reply}"
                        )
                        return formatted_reply
                    except Exception as format_error:
                        logger.error(
        ...
        try:
            ...
            for notification in notifications:
                ...
                try:
                    ...
                    match channel_type:
                        case "qq":
                            await self._send_qq_notification(
                                config_data, notification_msg
                            )
                    ...
                    logger.error(
                        f"发送通知失败 ({notification.get('channel_name', 'Unknown')}): {self._safe_str(notify_error)}"
                    )
    ...
    async def _send_dingtalk_notification(self, config_data: dict, message: str):
        ...
        try:
            ...
            data = {
                "msgtype": "markdown",
                "markdown": {"title": "闲鱼自动回复通知", "text": message},
            }
    ...
    async def send_token_refresh_notification(
        self, error_message: str, notification_type: str = "token_refresh"
    ):
        ...
        try:
            ...
            if current_time - last_time < self.notification_cooldown:
                logger.debug(
                    f"通知在冷却期内，跳过发送: {notification_type} (距离上次 {int(current_time - last_time)} 秒)"
                )
                return
...
请检查账号Cookie是否过期，如有需要请及时更新Cookie配置。"""
            ...
            for notification in notifications:
                ...
                try:
                    ...
                    logger.error(
                        f"发送Token刷新通知失败 ({notification.get('channel_name', 'Unknown')}): {self._safe_str(notify_error)}"
                    )
            ...
            if notification_sent:
                ...
                logger.info(
                    f"Token刷新通知已发送，下次可发送时间: {time.strftime('%H:%M:%S', time.localtime(current_time + self.notification_cooldown))}"
                )
    ...
    async def send_delivery_failure_notification(
        self, send_user_name: str, send_user_id: str, item_id: str, error_message: str
    ):
        """发送自动发货失败通知"""
        try:
            from db_manager import db_manager
                ...
                if notification.get("enabled", False):
                    ...
                    try:
                        ...
                        match channel_type:
                            case "qq":
                                await self._send_qq_notification(
                                    config_data, notification_message
                                )
                        ...
                        logger.error(
                            f"发送自动发货通知失败: {self._safe_str(notify_error)}"
                        )
    ...
    async def auto_confirm(self, order_id, retry_count=0):
        ...
        try:
            ...
            # 导入超级混淆加密模块
            from decrypted_secure_confirm_ultra import SecureConfirm
    ...
    async def auto_freeshipping(self, order_id, item_id, buyer_id, retry_count=0):
        ...
        try:
            ...
            # 导入超级混淆加密模块
            from decrypted_secure_freeshipping_ultra import SecureFreeshipping
            ...
            secure_freeshipping.refresh_token = (
                self.refresh_token
            )  # 传递refresh_token方法
            ...
            logger.error(
                f"【{self.cookie_id}】加密免拼发货模块调用失败: {self._safe_str(e)}"
            )
            return {
                "error": f"加密免拼发货模块调用失败: {self._safe_str(e)}",
                "order_id": order_id,
            }
    ...
    async def fetch_order_detail_info(self, order_id: str):
        ...
        try:
            ...
            result = await fetch_order_detail_simple(
                order_id, cookie_string, headless=True
            )
    ...
    async def _auto_delivery(
        self, item_id: str, item_title: str = None, order_id: str = None
    ):
        ...
        try:
            ...
            if item_id and item_id != "未知商品":
                ...
                try:
                    ...
                    if item_info and "data" in item_info:
                        ...
                        try:
                            ...
                            if content:
                                search_text = content
                                logger.info(
                                    f"API成功提取商品内容作为搜索文本: {content[:100]}..."
                                )
                            else:
                                search_text = shareInfoJsonString
                                logger.warning(
                                    "未能从API商品信息中提取到content字段，使用完整JSON字符串"
                                )
                        except json.JSONDecodeError as json_e:
                            logger.warning(
                                f"解析API商品信息JSON失败: {self._safe_str(json_e)}，使用原始字符串"
                            )
                            search_text = shareInfoJsonString
                        except Exception as parse_e:
                            logger.warning(
                                f"提取API商品内容失败: {self._safe_str(parse_e)}，使用原始字符串"
                            )
                            search_text = shareInfoJsonString
                    ...
                    logger.warning(
                        f"API获取商品信息失败: {self._safe_str(e)}，尝试从数据库获取"
                    )
                    ...
                    try:
                        ...
                        if db_item_info:
                            ...
                            if not item_detail_db.strip():
                                ...
                                if auto_fetch_config.get("enabled", True):
                                    logger.info(
                                        f"数据库中商品详情为空，尝试从外部API获取: {item_id}"
                                    )
                                    try:
                                        fetched_detail = (
                                            await self.fetch_item_detail_from_api(
                                                item_id
                                            )
                                        )
                                        if fetched_detail:
                                            ...
                                            logger.info(
                                                f"成功从外部API获取并保存商品详情: {item_id}"
                                            )
                                        else:
                                            logger.warning(
                                                f"外部API未能获取到商品详情: {item_id}"
                                            )
                                    except Exception as api_e:
                                        logger.warning(
                                            f"从外部API获取商品详情失败: {item_id}, 错误: {self._safe_str(api_e)}"
                                        )
                                else:
                                    logger.debug(
                            ...
                            if search_parts:
                                ...
                                logger.warning(
                                    f"数据库中商品标题和详情都为空，且无法从API获取: {item_id}"
                                )
                        ...
                        logger.debug(
                            f"从数据库获取商品信息失败: {self._safe_str(db_e)}"
                        )
                        search_text = item_title or item_id
            ...
            if is_multi_spec and order_id:
                ...
                try:
                    ...
                    logger.error(
                        f"获取订单规格信息失败: {self._safe_str(e)}，将使用兜底匹配"
                    )
            ...
            if spec_name and spec_value:
                logger.info(
                    f"尝试精确匹配多规格发货规则: {search_text[:50]}... [{spec_name}:{spec_value}]"
                )
                ...
                if delivery_rules:
                    logger.info(
                        f"✅ 找到精确匹配的多规格发货规则: {len(delivery_rules)}个"
                    )
                else:
            ...
            if not delivery_rules:
                ...
                if delivery_rules:
                    logger.info(
                        f"✅ 找到兜底匹配的普通发货规则: {len(delivery_rules)}个"
                    )
                else:
            ...
            if item_title_for_save:
                await self.save_item_info_to_db(
                    item_id, search_text, item_title_for_save
                )
            else:
            ...
            if rule.get("is_multi_spec"):
                if spec_name and spec_value:
                    logger.info(
                        f"🎯 精确匹配多规格发货规则: {rule['keyword']} -> {rule['card_name']} [{rule['spec_name']}:{rule['spec_value']}]"
                    )
                    logger.info(
                        f"📋 订单规格: {spec_name}:{spec_value} ✅ 匹配卡券规格: {rule['spec_name']}:{rule['spec_value']}"
                    )
                else:
                    logger.info(
                        f"⚠️ 使用多规格发货规则但无订单规格信息: {rule['keyword']} -> {rule['card_name']} [{rule['spec_name']}:{rule['spec_value']}]"
                    )
            else:
                if spec_name and spec_value:
                    logger.info(
                        f"🔄 兜底匹配普通发货规则: {rule['keyword']} -> {rule['card_name']} ({rule['card_type']})"
                    )
                    logger.info(
                        f"📋 订单规格: {spec_name}:{spec_value} ➡️ 使用普通卡券兜底"
                    )
                else:
                    logger.info(
                        f"✅ 匹配普通发货规则: {rule['keyword']} -> {rule['card_name']} ({rule['card_type']})"
                    )
                ...
                if not self.is_auto_confirm_enabled():
                    ...
                    if should_confirm:
                        ...
                        if confirm_result.get("success"):
                            ...
                            logger.warning(
                                f"⚠️ 自动确认发货失败: {confirm_result.get('error', '未知错误')}"
                            )
                            # 即使确认发货失败，也继续发送发货内容
            ...
            if order_id:
                ...
                if delivery_content:
                    ...
                    logger.info(
                        f"自动发货成功: 规则ID={rule['id']}, 内容长度={len(final_content)}"
                    )
        ...
        while True:
            try:
                ...
                if (
                    ...
                    if new_token:
                        logger.info(
                            f"【{self.cookie_id}】Token刷新成功，准备重新建立连接..."
                        )
    ...
    async def _create_websocket_connection(self, headers):
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    "websockets库不支持extra_headers参数，尝试additional_headers"
                )
                ...
                try:
                    ...
                    if (
                        ...
                        # 如果都不支持，则不传递headers
                        logger.warning(
                            "websockets库不支持headers参数，使用基础连接模式"
                        )
        ...
        try:
                    ...
                    if isinstance(parsed_data, dict) and "chatType" in parsed_data:
                        if (
                            ...
                            if "sessionArouse" in content:
                                # 处理系统引导消息
                                logger.info(
                                    f"[{msg_time}] 【{self.cookie_id}】【系统】小闲鱼智能提示:"
                                )
                                ...
                                # 其他类型的未加密消息
                                logger.debug(
                                    f"[{msg_time}] 【{self.cookie_id}】【系统】其他类型消息: {content}"
                                )
                        return
                    else:
            ...
            try:
                ...
                if red_reminder == "等待买家付款":
                    ...
                    user_url = f"https://www.goofish.com/personal?userId={user_id}"
                    logger.info(
                        f"[{msg_time}] 【系统】交易成功 {user_url} 等待卖家发货"
                    )
                    # return
            except:
                pass
            ...
            if send_user_id == self.myid:
                logger.info(
                    f"[{msg_time}] 【手动发出】 商品({item_id}): {send_message}"
                )
            ...
            if send_message == "[我已拍下，待付款]":
                ...
                send_message == "快给ta一个评价吧~"
                or send_message == "快给ta一个评价吧～"
            ):
            ...
            # 检查是否为自动发货触发消息
                ...
                try:
                    ...
                    if card_title == "我已小刀，待刀成":
                        logger.info(
                            f'[{msg_time}] 【{self.cookie_id}】【系统】检测到"我已小刀，待刀成"，准备自动免拼发货'
                        )
                        # 提取订单ID
                        order_id = self._extract_order_id(message)
                        if order_id:
                ...
                if reply:
                    ...
                    if reply:
                        ...
                        reply = await self.get_default_reply(
                            send_user_name, send_user_id, send_message
                        )
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.info(
                        f"【{self.cookie_id}】准备建立WebSocket连接到: {self.base_url}"
                    )
                ...
                if res_json.get("ret") and res_json["ret"][0] == "SUCCESS::调用成功":
                    ...
                    if (
                        ...
                        return await self.get_item_list_info(
                            page_number, page_size, retry_count + 1
                        )
                    else:

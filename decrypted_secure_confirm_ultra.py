...
class SecureConfirm:
    ...
    async def update_config_cookies(self):
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】更新数据库Cookie失败: {self._safe_str(e)}"
            )
        ...
        try:
            ...
            async with self.session.post(
                ...
                if res_json.get("ret") and res_json["ret"][0] == "SUCCESS::调用成功":
                    logger.info(
                        f"【{self.cookie_id}】✅ 自动确认发货成功，订单ID: {order_id}"
                    )
                    ...
                    logger.warning(
                        f"【{self.cookie_id}】❌ 自动确认发货失败: {error_msg}"
                    )
            ...
            logger.error(
                f"【{self.cookie_id}】自动确认发货API请求异常: {self._safe_str(e)}"
            )

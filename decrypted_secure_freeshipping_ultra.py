...
class SecureFreeshipping:
    ...
    async def update_config_cookies(self):
        ...
        try:
            ...
            logger.error(
                f"【{self.cookie_id}】更新Cookie到数据库失败: {self._safe_str(e)}"
            )
    ...
    async def auto_freeshipping(self, order_id, item_id, buyer_id, retry_count=0):
        ...
        logger.info(
            f"【{self.cookie_id}】参数详情 - order_id: {order_id}, item_id: {item_id}, buyer_id: {buyer_id}"
        )
        ...
        try:
            ...
            async with self.session.post(
                ...
                if res_json.get("ret") and res_json["ret"][0] == "SUCCESS::调用成功":
                    logger.info(
                        f"【{self.cookie_id}】✅ 自动免拼发货成功，订单ID: {order_id}"
                    )
                    return {"success": True, "order_id": order_id}
                else:
                    error_msg = (
                        res_json.get("ret", ["未知错误"])[0]
                        if res_json.get("ret")
                        else "未知错误"
                    )
                    logger.warning(
                        f"【{self.cookie_id}】❌ 自动免拼发货失败: {error_msg}"
                    )

                    # 如果是token相关错误，进行重试
                    if "token" in error_msg.lower() or "sign" in error_msg.lower():
                        logger.info(f"【{self.cookie_id}】检测到token错误，准备重试...")
                        return await self.auto_freeshipping(
                            order_id, item_id, buyer_id, retry_count + 1
                        )
            ...
            logger.error(
                f"【{self.cookie_id}】自动免拼发货API请求异常: {self._safe_str(e)}"
            )

...
注意：结合商品信息，给出实用建议。""",
    ...
    def detect_intent(self, message: str, cookie_id: str) -> str:
        ...
        try:
            settings = db_manager.get_ai_reply_settings(cookie_id)
            custom_prompts = (
                json.loads(settings["custom_prompts"])
                if settings["custom_prompts"]
                else {}
            )
            classify_prompt = custom_prompts.get(
                "classify", self.default_prompts["classify"]
            )
...
请根据以上信息生成回复："""
    ...
    def get_conversation_context(
        self, chat_id: str, cookie_id: str, limit: int = 20
    ) -> List[Dict]:
        """获取对话上下文"""
        try:
            with db_manager.lock:
                ...
                # 反转顺序，使其按时间正序
                context = [
                    {"role": row[0], "content": row[1]} for row in reversed(results)
                ]
                return context
        except Exception as e:
